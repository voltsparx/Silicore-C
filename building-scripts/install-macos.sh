...
DEFAULT_INSTALL_DIR="/usr/local/opt/silicore-c"
SYMLINK_PATH="/usr/local/bin/silicore-c"
BIN_NAME="silicore-c"
...
install_binary() {
  ...
  run_privileged install -m 755 "$BIN_PATH" "${INSTALL_DIR}/${BIN_NAME}"
  run_privileged mkdir -p "$(dirname "$SYMLINK_PATH")"
  run_privileged ln -sf "${INSTALL_DIR}/${BIN_NAME}" "$SYMLINK_PATH"
}
...
mode_uninstall() {
  ...
  if [[ -e "$SYMLINK_PATH" ]]; then
    run_privileged rm -f "$SYMLINK_PATH"
  fi
